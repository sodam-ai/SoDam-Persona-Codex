...
이 플러그인 자체는 별도의 AI가 아닙니다. Codex가 이미 갖고 있는 대화 능력 위에 "이렇게 판단하고 이렇게 답하라"는 규칙 문서를 자동으로 얹어주는 설정 모음입니다. Codex가 해당 이벤트를 전달할 때 실행되도록 설정된 hook(후크 — 특정 시점에 실행되는 작은 프로그램) 2개와, 상황에 맞을 때 불러오는 전문 지식 모음(skill, 스킬) 31개로 이루어져 있습니다. 설치·권한·환경에 따라 실제 자동 실행 여부를 확인해야 합니다.

> **이 작업 브랜치 버전**: `1.10.2` · **관점 수**: 37명 · **트리거 패턴**: 43개(A~AQ) · **스킬 수**: 31개 · **hook 수**: 2개 · **라이선스**: Apache License 2.0

> **설치 전 확인**: 기본 GitHub 설치와 ZIP 다운로드는 기본 브랜치의 내용을 가져옵니다. 2026-09-23 현재 이 v1.10.2 작업 브랜치(`codex/fix-codex-global-hooks-v1.10.2`)와 기본 브랜치는 다른 커밋입니다. 기본 설치가 v1.10.2라고 가정하지 마세요. 아래 시험용 설치 절을 따르세요. 작업 브랜치는 정식 배포가 아닙니다.
...
**v1.10.2 검증 기록(2026-09-23, Windows)**: 작업 브랜치에서 정합성 검사, hook·검증기 자동 테스트 총 22건, 공식 플러그인 구조 검사, 격리된 임시 환경의 설치·직접 hook 실행, 한·영 HTML 재생성과 Chrome 화면·콘솔 검사가 통과했습니다. 기존 개인 설치본 갱신이나 일반 자연어 요청에서 hook 자동 호출까지 입증한 것은 아닙니다.

**미확인 범위**: 기존 개인 설치본은 마지막 확인 시 v1.10.1이었습니다. 일반 자연어 요청에서 v1.10.2 hook 자동 호출, macOS/Linux 실기기, 모든 Codex 앱·IDE 조합은 미검증입니다. CI·브라우저 검사도 해당 커밋과 환경에 한정됩니다. [설치 확인](#설치-확인)을 각 환경에서 진행하세요.
...
### v1.10.2 작업 브랜치 설치 (시험용)

마켓플레이스를 아직 등록하지 않은 **별도 시험 환경**에서만 다음 명령을 사용하세요. Codex CLI 도움말에서 `--ref` 옵션을 확인했습니다.
...
이미 `sodam-persona`가 등록되어 있다면 중복 등록하지 말고 `codex plugin marketplace list`로 확인하세요. 기존 설치를 지우지 않고 시험하려면 별도 Codex 환경을 사용하세요. 또는 `git clone --branch codex/fix-codex-global-hooks-v1.10.2 https://github.com/sodam-ai/SoDam-Persona-Codex.git`으로 내려받아 충돌 없는 환경에서 로컬 설치 방법을 따르세요.
...
| 13 | 실제 직렬화된 hook 출력이 저장소 자체 12,000자 상한을 넘지 않는가 |
...
`.github/workflows/validate.yml`은 `main` push와 Pull Request에서 정합성 검사와 두 회귀 테스트를 실행하도록 설정되어 있습니다. CI 실패가 병합을 차단하는지는 GitHub 브랜치 보호 규칙에 달려 있습니다.
...
- `persona-registry.json`을 관점·패턴·도메인·버전·hook 상한의 구조화 정본으로 연결했습니다.
- SessionStart 직렬화 출력을 14,153자에서 12,000자 미만으로 줄이고 검사 상한도 12,000자로 고정했습니다.
- `node diagnose.mjs`로 소스와 설치본의 버전 차이, 활성 상태, hook 실행, 저장소 정합성을 한 번에 확인할 수 있습니다.
- 배포 전 최종 점검에서 커밋된 HEAD만 별도 폴더에 풀어 전체 테스트·문서 빌드·플러그인 구조 검사를 다시 통과했고, 실제로 실행하지 못한 원격 CI와 브라우저 화면 검증은 완료된 것처럼 쓰지 않도록 문서를 바로잡았습니다.
- Apache License 2.0 공식 원문 일치 여부, NOTICE의 제3자 문구, 외부 자산·의존성·민감정보를 다시 점검하고, 프로젝트 라이선스가 제3자 자료나 외부 AI 결과물의 권리까지 보장하지 않는다는 경계를 명확히 했습니다.
...
| 세션이 길어진 뒤 페르소나가 흐트러진 느낌 | 해당 환경에서 hook이 실행되지 않았거나 마커의 지시가 충분히 반영되지 않았을 수 있음 | `codex plugin list` 확인 → 새 task에서 재현 → 계속되면 `node diagnose.mjs` 결과와 재현 문장을 Issue에 기록 |
...
A. 아니요. 이 페르소나는 실제 자격을 갖춘 세무사·회계사·변호사가 아닙니다. 참고용 정보 제공이 목적이며, 실행성 있는 판단(신고·계약 해석 등) 전에는 실제 전문가의 확인을 받아야 합니다. 면책 문구는 규칙에 필수로 지정되어 있지만 모든 실제 답변에서 표시되는지는 사용자가 확인해야 합니다.
...
A. 여러 컴퓨터에 설치할 수 있습니다. 하지만 Codex 버전·계정 권한·Node.js·조직 정책·설치 버전에 따라 동작이 달라질 수 있으므로 각 컴퓨터에서 [설치 확인](#설치-확인)을 진행하세요.
...
A. `UserPromptSubmit` hook은 해당 이벤트가 전달될 때 마커를 재주입하도록 설계되어 있습니다. compaction·서브에이전트·모든 Codex 환경에서 실제 전달되는지는 미검증이므로 자동 복구를 보장할 수 없습니다. 새 task에서 확인하세요.
...
| `NOTICE`의 관련 고지 포함 | 원본에 `NOTICE`가 있으므로 Apache License 2.0 제4조(d)에 따라 배포하는 부분에 해당하는 귀속 고지를 읽을 수 있는 형태로 제공해야 합니다(`NOTICE`, 문서 또는 화면 표시 등). 원문과 적용 범위를 확인하세요 |

그리고 아래는 **보장되지 않는다는 점**을 분명히 알려드립니다(원문 제7·8조 요약).

- 이 소프트웨어는 **"있는 그대로(AS IS)"** 제공되며, 상품성이나 특정 목적 적합성을 포함해 어떠한 형태의 보증도 하지 않습니다.
- 원문 제8조는 적용 법률에서 요구하거나 별도 서면 합의한 경우 등을 제외하고 책임을 제한합니다. 모든 관할·모든 상황에서 손해배상 책임이 절대 없다고 단정할 수는 없습니다.
- 사용 및 재배포의 적합성을 판단하고 그에 따른 위험을 부담하는 것은 전적으로 사용자의 책임입니다.

### Apache License 2.0이 실제로 적용되는 범위

**왕초보 한 줄 정리**: 이 저장소에 Apache License 2.0이 표시되어 있어도, 그 라이선스는 SoDam AI Studio와 각 기여자가 적법하게 허락할 수 있는 권리에만 적용됩니다. 다른 사람의 글·상표·이미지·고객 자료·AI 서비스 결과물까지 자동으로 허락해 주는 문서는 아닙니다.

| 대상 | 현재 확인된 상태 | 공개·상업적 사용 전 해야 할 일 |
|---|---|---|
| 이 프로젝트가 직접 작성한 코드·문서·프롬프트·규칙 | Apache License 2.0 적용이 표시되어 있음. 다만 저작권자 법적 주체명, 기여자 권리 이전, AI 보조 작성물의 권리 사슬은 독립적으로 입증하지 않음 | `LICENSE`와 관련 `NOTICE` 고지를 유지하고, 수정 파일에 변경 사실을 표시. 권리 보증이 필요한 납품은 **법무/전문가 검토 필요** |
| `persona-triggers`에 남아 있는 제3자 문구 3건 | 출처만 식별됨. 별도 허락·라이선스·관할별 인용 예외 적용 여부는 확인되지 않음 | 공개·상업 재배포 전에 허락, 적용 가능한 라이선스 또는 인용 예외 등 적법한 근거를 기록. 근거가 불명확하면 **법무/전문가 검토 필요** |
| 제품명·회사명·상표 | 호환성·출처 설명을 위한 문자 표기만 있으며 로고 파일은 없음 | 제휴·후원·공식 승인을 암시하지 말고, 원본 출처 설명에 필요한 범위만 사용 |
| Codex 등 외부 AI 서비스의 입력·출력 | 이 프로젝트의 Apache License 2.0만으로 권리나 상업적 이용 가능 여부를 보장할 수 없음 | 입력 자료의 권리·동의, 출력의 유사성·침해 가능성, 계정 유형별 최신 약관과 정책을 사용자가 직접 확인 |
| 사용자가 넣는 고객 자료·내부 문서·개인정보 | 현재 저장소에는 실제 자료가 없지만, 설치 후 사용자가 입력할 수 있음 | 업로드·처리 권한, 비밀유지 의무, 개인정보 처리 근거와 고객 동의를 먼저 확인 |

`NOTICE`에 출처를 적는 것은 **출처 표시**일 뿐, 제3자 자료의 이용 허락을 새로 만들어 주지 않습니다. 또한 Apache License 2.0 제5조에 따라 프로젝트에 포함할 목적으로 의도적으로 제출한 기여물은 별도 표시나 계약이 없다면 같은 라이선스 조건으로 제출된 것으로 다뤄질 수 있으므로, 기여자는 제출 권한이 있는 자료만 제공해야 합니다.
...
- 개인용 서비스 약관: `https://openai.com/policies/terms-of-use/`
- 기업·개발자·API용 서비스 계약: `https://openai.com/policies/services-agreement/`
- 사용 정책: `https://openai.com/policies/usage-policies/`
- 개인정보처리방침: `https://openai.com/policies/privacy-policy/`
- 공식 플러그인 설치·권한 안내: `https://help.openai.com/en/articles/20001256/`

위 링크와 정책은 바뀔 수 있으므로 실제 사용·배포·납품 시점에 다시 확인해야 합니다.

### 저장소에 포함된 자산·외부 의존성 점검

이 저장소를 코드·문서 전체 기준으로 점검한 결과(2026-09-23, Git 추적 대상 64개 파일 기준)입니다.
...
| 샘플 계정, 이메일, 전화번호, 실제 사용자 홈 경로, 실제 고객 데이터 | Git 추적 대상 64개 파일에서 실제 데이터 **0건 확인**. 검증기의 차단 기능을 시험하기 위한 일반화된 가짜 Windows 사용자 경로 문자열 1개만 차단 확인용 테스트에 포함(실제 사용자 정보 아님) |
...
**현재 공개·납품 판단**: `plugins/sodam-persona/skills/persona-triggers/SKILL.md`의 제3자 문구 3건은 출처가 표시되어 있지만, 이용 허락·라이선스·인용 예외에 따른 사용 가능성은 확인되지 않았습니다. 이 문구가 포함된 저장소 전체를 공개 재배포·판매·고객사 납품할 때는 권리 검토가 끝났다고 표시하지 마세요. 권리 근거를 확인·기록하거나 법무/전문가 검토를 받은 뒤 결정해야 합니다. 이는 Apache License 2.0에 새로운 조건을 추가하는 것이 아니라, 별도 제3자 권리에 대한 미확인 사항입니다.

| 하고 싶은 것 | Apache 2.0상 가능 여부 | 조건·별도 확인 사항 |
|---|---|---|
...
| 우선순위 | 해야 할 일 |
|---|---|
| **Must Have** | `LICENSE` 사본 제공, `NOTICE`의 관련 고지 전달, 수정 파일에 변경 사실 표시, 상표로 제휴·후원을 오인시키지 않기, 외부 서비스 약관·개인정보·고객 자료 사용 동의 확인. 공개 재배포·판매·납품 전에는 제3자 문구 3건의 사용 권리 근거도 확인·기록하거나 법무/전문가 검토 받기 |
| **Should Have** | 제3자 인용의 정확한 출처와 적용 범위를 기록하고, 저작권자 표기의 법적 주체명·기여자 권리 이전·AI 보조 작성물의 권리 귀속 기록을 정리하기 |
