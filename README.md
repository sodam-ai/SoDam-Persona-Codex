...
> **지금 버전**: `1.5.0` · **관점 수**: 22명 · **트리거 패턴**: 27개(A~AA) · **스킬 수**: 16개 · **hook 수**: 2개 · **라이선스**: Apache License 2.0
...
이 트리거 단어들은 27개의 패턴 그룹(A~AA)으로 정리되어 있으며, 자세한 단어 목록과 충돌 시 우선순위는 `persona-triggers` skill에 전부 정리되어 있습니다. 우선순위 요약: **분량 제약(간단히 등) > 강도(깊게 등) > 도메인 전문가 > 단일 관점 > 추가 skill.**
...
└── plugins/sodam-persona/                # 실제로 배포·설치되는 플러그인 본체
    ...
    │   ├── persona-triggers/SKILL.md     # 트리거 단어 A~AA 상세, 22개 관점 매핑표
...
| 3 | 트리거 패턴 ID(A~AA) 개수 표기가 실제 섹션 수와 일치하는가 |
...
- Agent Plugins 1.0 루트 `plugin.json`을 정본으로 추가하고 `.codex-plugin/plugin.json`을 fallback으로 유지했습니다. 현재 세 매니페스트의 버전은 `1.5.0`으로 통일했습니다.
