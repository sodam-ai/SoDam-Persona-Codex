...
`.github/workflows/validate.yml`이 `main` 브랜치로의 push와 모든 Pull Request마다 정합성 검사와 두 실행 회귀 테스트를 자동으로 실행해, 오류가 있는 변경이 `main`에 들어오지 못하도록 막습니다.
...
이 저장소를 코드·문서 전체 기준으로 점검한 결과(2026-09-17, Git 추적 대상 64개 파일 기준)입니다.
