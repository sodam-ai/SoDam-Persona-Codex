...
> **지금 버전**: `1.4.0` · **관점 수**: 20명 · **트리거 패턴**: 25개(A~Y) · **스킬 수**: 14개 · **hook 수**: 2개 · **라이선스**: Apache License 2.0
...
| **L3** | 보안·금전·배포·되돌릴 수 없는 작업이 섞인 중대 작업 | `persona-format` + `persona-triggers` + `persona-safety` 전부 활성화 + 20명 관점 전원 검토 |
...
이 트리거 단어들은 25개의 패턴 그룹(A~Y)으로 정리되어 있으며, 자세한 단어 목록과 충돌 시 우선순위는 `persona-triggers` skill에 전부 정리되어 있습니다. 우선순위 요약: **분량 제약(간단히 등) > 강도(깊게 등) > 도메인 전문가 > 단일 관점 > 추가 skill.**

### 20명의 관점 — 매 응답 전 자동으로 검토되는 전문가 체크리스트

L1 이상의 모든 응답에서, 아래 20개 관점 중 그 작업과 관련 있는 3~5개를 짧게라도 내부적으로 검토한 뒤 답합니다(트리거 단어가 없어도 자동 적용됩니다). L0 잡담이나 "간단히"류 요청에는 적용되지 않습니다.
...
### 도메인 전문가 9종 — 조건부로 깊게 들어가는 전문 지식

위 20개 관점 중 #11·#13·#14·#15·#16·#17·#18·#19·#20은 전용 skill을 따로 갖고 있어, 관련 트리거가 감지되면 훨씬 깊은 지식이 추가로 로드됩니다.
...
6. 조건에 맞는 skill 조건부 로드
   (persona-triggers / persona-format / persona-safety / 도메인 9종 중 해당하는 것)
       │
       ▼
...
└── plugins/sodam-persona/                # 실제로 배포·설치되는 플러그인 본체
    ...
    │   ├── persona-triggers/SKILL.md     # 트리거 단어 A~Y 상세, 20개 관점 매핑표
...
| 2 | "20명"·"20관점" 같은 표기가 모든 핵심 파일(코어·마커·skill·README 등)에서 실제 관점 수와 일치하는가 |
| 3 | 트리거 패턴 알파벳(A~Y) 개수 표기가 실제 섹션 수와 일치하는가 |
| 4 | skill 폴더 수와 문서상 표기가 일치하고, 각 skill의 frontmatter `name`이 폴더명과 같은가(영문 README 교차검사 포함) |
| 5 | 도메인 페르소나 9종이 코어와 마커 파일 양쪽에 모두 배선되어 있는가 |
| 6 | Agent Plugins 1.0·Codex fallback·legacy 매니페스트와 두 마켓플레이스 JSON의 이름·버전·소스 경로가 올바른가 |
| 7 | 9개 도메인의 필수 면책·자격자 확인·확정 금액 경계가 실제로 존재하는가 |
...
| 14 | 코어 트리거가 정본과 일치하며, 건축·인테리어 5종이 공통 협업 규칙을 참조하고 일반어 단독 트리거를 피하는가 |
...
| 도메인 전문가 9종 상세 | `plugins/sodam-persona/skills/persona-investor|lawyer|accountant|marketer|architectural-designer|interior-designer|construction-expert|cost-estimator|design-director/SKILL.md` |
...
- 관점 20개, 트리거 패턴 25개(A~Y), 스킬 14개로 확장하고 hook·문서·검증기를 동기화했습니다.
...
- Agent Plugins 1.0 루트 `plugin.json`을 정본으로 추가하고 `.codex-plugin/plugin.json`을 fallback으로 유지했습니다. 세 매니페스트의 버전은 `1.4.0`으로 통일했습니다.
...
A. "객관적으로", "깊게", "철저히", "페르소나 풀버전" 같은 표현을 사용하면 20명 관점 전원이 검토에 들어갑니다.
