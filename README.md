...
이 플러그인 자체는 별도의 AI가 아닙니다. Codex가 이미 갖고 있는 대화 능력 위에 "이렇게 판단하고 이렇게 답하라"는 규칙 문서를 자동으로 얹어주는 설정 모음입니다. 항상 켜지는 핵심 규칙(hook, 후크 — 특정 시점에 자동으로 실행되는 작은 프로그램) 2개와, 상황에 맞을 때만 불러오는 전문 지식 모음(skill, 스킬) 21개로 이루어져 있습니다.

> **지금 버전**: `1.7.0` · **관점 수**: 27명 · **트리거 패턴**: 33개(A~AG) · **스킬 수**: 21개 · **hook 수**: 2개 · **라이선스**: Apache License 2.0
...
| `$persona-create` | 인터뷰 방식으로 새 도메인 페르소나를 추가. 현재 번호 체계에서는 다음 관점이 #28부터 시작 |
...
| **L3** | 보안·금전·배포·되돌릴 수 없는 작업이 섞인 중대 작업 | `persona-format` + `persona-triggers` + `persona-safety` 전부 활성화 + 27명 관점 전원 검토 |
...
4. **도메인 전문가 활성화** — 법률 → #11, 투자 → #13, 회계·세무 → #14, 마케팅 → #15, 건축·인테리어·시공·견적·디자인·3D·렌더링 → #16~#24, 자료 검색·출처 검증 → #25, 리서치·분석 → #26, 아이디어·콘셉트 → #27
...
이 트리거 단어들은 33개의 패턴 그룹(A~AG)으로 정리되어 있으며, 자세한 단어 목록과 충돌 시 우선순위는 `persona-triggers` skill에 전부 정리되어 있습니다. 우선순위 요약: **분량 제약(간단히 등) > 강도(깊게 등) > 도메인 전문가 > 단일 관점 > 추가 skill.**

### 27명의 관점 — 매 응답 전 자동으로 검토되는 전문가 체크리스트

L1 이상의 모든 응답에서, 아래 27개 관점 중 그 작업과 관련 있는 3~5개를 짧게라도 내부적으로 검토한 뒤 답합니다(트리거 단어가 없어도 자동 적용됩니다). L0 잡담이나 "간단히"류 요청에는 적용되지 않습니다.
...
### 도메인 전문가 16종 — 조건부로 깊게 들어가는 전문 지식

위 27개 관점 중 #11·#13~#27은 전용 skill을 따로 갖고 있어, 관련 트리거가 감지되면 훨씬 깊은 지식이 추가로 로드됩니다.
...
6. 조건에 맞는 skill 조건부 로드
   (persona-triggers / persona-format / persona-safety / 도메인 16종 중 해당하는 것)
       │
       ▼
...
└── plugins/sodam-persona/                # 실제로 배포·설치되는 플러그인 본체
    ...
    │   ├── persona-triggers/SKILL.md     # 트리거 단어 A~AG 상세, 27개 관점 매핑표
...
| 2 | "27명"·"27관점" 같은 표기가 모든 핵심 파일(코어·마커·skill·README 등)에서 실제 관점 수와 일치하는가 |
| 3 | 트리거 패턴 ID(A~AG) 개수 표기가 실제 섹션 수와 일치하는가 |
| 4 | skill 폴더 수·README 첫 설명·파일 위치 표가 일치하고, 각 skill의 frontmatter `name`이 폴더명과 같은가(영문 README 교차검사 포함) |
| 5 | 도메인 페르소나 16종이 코어·마커 양쪽에 배선되고, 한영 README의 도메인 수·명시 호출 명령·주요 목차가 일치하는가 |
| 6 | Agent Plugins 1.0·Codex fallback·legacy 매니페스트와 두 마켓플레이스 JSON의 이름·버전·소스 경로가 올바른가 |
| 7 | 16개 도메인의 필수 면책·자격자 확인·확정 금액 경계가 실제로 존재하는가 |
...
| 21개 skill 전체 | `plugins/sodam-persona/skills/persona-*/SKILL.md` |
| 트리거 단어 전체 목록과 관점 매핑표 | `plugins/sodam-persona/skills/persona-triggers/SKILL.md` |
| 도메인 전문가 16종 상세 | `plugins/sodam-persona/skills/` 아래 16개 도메인 `persona-*` 폴더 |
...
- 파일·코드·UI·데이터베이스·SEO 검색, 감탄, IDE 언급으로 잘못 켜지는 상황을 제외하고 27관점·33패턴·21스킬 정합성 검사를 보강했습니다.
...
A. "객관적으로", "깊게", "철저히", "페르소나 풀버전" 같은 표현을 사용하면 27명 관점 전원이 검토에 들어갑니다.
