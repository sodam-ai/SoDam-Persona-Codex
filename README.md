...
이 플러그인 자체는 별도의 AI가 아닙니다. Codex가 이미 갖고 있는 대화 능력 위에 "이렇게 판단하고 이렇게 답하라"는 규칙 문서를 자동으로 얹어주는 설정 모음입니다. 항상 켜지는 핵심 규칙(hook, 후크 — 특정 시점에 자동으로 실행되는 작은 프로그램) 2개와, 상황에 맞을 때만 불러오는 전문 지식 모음(skill, 스킬) 29개로 이루어져 있습니다.

> **지금 버전**: `1.9.0` · **관점 수**: 35명 · **트리거 패턴**: 41개(A~AO) · **스킬 수**: 29개 · **hook 수**: 2개 · **라이선스**: Apache License 2.0
...
| `$persona-create` | 인터뷰 방식으로 새 도메인 페르소나를 추가. 현재 번호 체계에서는 다음 관점이 #36부터 시작 |
...
| **L3** | 보안·금전·배포·되돌릴 수 없는 작업이 섞인 중대 작업 | `persona-format` + `persona-triggers` + `persona-safety` 전부 활성화 + 35명 관점 전원 검토 |
...
4. **도메인 전문가 활성화** — 법률 → #11, 투자 → #13, 회계·세무 → #14, 마케팅 → #15, 건축·인테리어·시공·견적·디자인·3D·렌더링 → #16~#24, 자료 검색·출처 검증 → #25, 리서치·분석 → #26, 아이디어·콘셉트 → #27, 프로젝트 실행관리 → #28, 제품 가치·백로그 → #29, PMO·거버넌스 → #30, 프로젝트 분석·운영 → #31, 이미지 제작·편집 → #32, 영상 제작·연출 → #33, 영상 편집·후반작업 → #34, 미디어 품질·권리 검수 → #35
...
이 트리거 단어들은 41개의 패턴 그룹(A~AO)으로 정리되어 있으며, 자세한 단어 목록과 충돌 시 우선순위는 `persona-triggers` skill에 전부 정리되어 있습니다. 우선순위 요약: **분량 제약(간단히 등) > 강도(깊게 등) > 도메인 전문가 > 단일 관점 > 추가 skill.**

### 35명의 관점 — 매 응답 전 자동으로 검토되는 전문가 체크리스트

L1 이상의 모든 응답에서, 아래 35개 관점 중 그 작업과 관련 있는 3~5개를 짧게라도 내부적으로 검토한 뒤 답합니다(트리거 단어가 없어도 자동 적용됩니다). L0 잡담이나 "간단히"류 요청에는 적용되지 않습니다.
...
### 도메인 전문가 24종 — 조건부로 깊게 들어가는 전문 지식

위 35개 관점 중 #11·#13~#35은 전용 skill을 따로 갖고 있어, 관련 트리거가 감지되면 훨씬 깊은 지식이 추가로 로드됩니다.
...
6. 조건에 맞는 skill 조건부 로드
   (persona-triggers / persona-format / persona-safety / 도메인 24종 중 해당하는 것)
       │
       ▼
...
└── plugins/sodam-persona/                # 실제로 배포·설치되는 플러그인 본체
    ...
    │   ├── persona-triggers/SKILL.md     # 트리거 단어 A~AO 상세, 35개 관점 매핑표
    ...
    └── reference/
        ...
        ├── media_production_collaboration.md # 건축·인테리어 공통 입력·인계·충돌 규칙
...
| 2 | "35명"·"35관점" 같은 표기가 모든 핵심 파일(코어·마커·skill·README 등)에서 실제 관점 수와 일치하는가 |
| 3 | 트리거 패턴 ID(A~AO) 개수 표기가 실제 섹션 수와 일치하는가 |
...
| 29개 skill 전체 | `plugins/sodam-persona/skills/persona-*/SKILL.md` |
| 트리거 단어 전체 목록과 관점 매핑표 | `plugins/sodam-persona/skills/persona-triggers/SKILL.md` |
| 도메인 전문가 24종 상세 | `plugins/sodam-persona/skills/` 아래 24개 도메인 `persona-*` 폴더 |
...
| 이미지·영상 요청이 일반 디자인이나 렌더링 역할로만 처리됨 | 설치 캐시가 v1.9.0 이전이거나 “이미지·영상”만 입력해 목적이 불명확함 | 최신 버전을 다시 설치하고 이미지 합성·워크스루 스토리보드·영상 편집·품질/라이선스 검수처럼 작업을 적거나 #32~#35를 직접 호출 |
...
A. "객관적으로", "깊게", "철저히", "페르소나 풀버전" 같은 표현을 사용하면 35명 관점 전원이 검토에 들어갑니다.
