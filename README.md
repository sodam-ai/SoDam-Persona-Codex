...
이 플러그인 자체는 별도의 AI가 아닙니다. Codex가 이미 갖고 있는 대화 능력 위에 "이렇게 판단하고 이렇게 답하라"는 규칙 문서를 자동으로 얹어주는 설정 모음입니다. 항상 켜지는 핵심 규칙(hook, 후크 — 특정 시점에 자동으로 실행되는 작은 프로그램) 2개와, 상황에 맞을 때만 불러오는 전문 지식 모음(skill, 스킬) 16개로 이루어져 있습니다.
...
| 20 | 건축·인테리어 디자인 디렉터(15년+) | 공간 콘셉트·재료·디자인 일관성이 걸린 작업 |
...
6. 조건에 맞는 skill 조건부 로드
   (persona-triggers / persona-format / persona-safety / 도메인 11종 중 해당하는 것)
       │
       ▼
...
| 5 | 도메인 페르소나 11종이 코어와 마커 파일 양쪽에 모두 배선되어 있는가 |
| 6 | Agent Plugins 1.0·Codex fallback·legacy 매니페스트와 두 마켓플레이스 JSON의 이름·버전·소스 경로가 올바른가 |
| 7 | 11개 도메인의 필수 면책·자격자 확인·확정 금액 경계가 실제로 존재하는가 |
...
| 14 | 코어 트리거가 정본과 일치하며, 건축·인테리어 7종이 공통 협업 규칙을 참조하고 일반어 단독 트리거를 피하는가 |
...
| 14개 skill 전체 | `plugins/sodam-persona/skills/persona-*/SKILL.md` |
| 트리거 단어 전체 목록과 관점 매핑표 | `plugins/sodam-persona/skills/persona-triggers/SKILL.md` |
| 도메인 전문가 11종 상세 | `plugins/sodam-persona/skills/` 아래 11개 도메인 `persona-*` 폴더 |
