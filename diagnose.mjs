...
const manifest = JSON.parse(readFileSync(join(ROOT, 'plugins', 'sodam-persona', 'plugin.json'), 'utf8'));
...
function installedVersion() {
  ...
  const row = text.match(/^\s*sodam-persona@sodam-persona\s+(.+?)\s+(\d+\.\d+\.\d+)\s+.+$/mi);
...
const mismatch = installed.status === 'found' && installed.version !== manifest.version;
...
console.log(`- SessionStart 훅: ${core.ok ? `정상 (${core.chars}자)` : `실패 — ${core.error}`}`);
console.log(`- UserPromptSubmit 훅: ${marker.ok ? `정상 (${marker.chars}자)` : `실패 — ${marker.error}`}`);
