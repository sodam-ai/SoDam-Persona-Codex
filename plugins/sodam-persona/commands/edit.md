---
description: "인터뷰 방식으로 기존 페르소나(현재 37관점, 도메인 26종 포함)의 트리거 단어를 추가·수정·제거"
---
...
- **도메인 페르소나** (#11 법률, #13 투자자, #14 회계세무, #15 마케팅, #16~#24 건축·인테리어·3D 계열, #25~#27 검색·리서치·아이디어, #28~#31 프로젝트관리, #32~#35 이미지·영상, #36~#37 생성형 AI 도구·플랫폼, 그리고 `create`로 만들어진 것들 — 전용 스킬 파일이 있고 `persona-triggers/SKILL.md`에 별도 알파벳 섹션이 있음) → 아래 5곳을 **전부 동기화**해서 편집해야 함:
  1. `persona-triggers/SKILL.md`의 해당 도메인 알파벳 섹션(J/K/S/T/U~AQ/…) 트리거 단어군. AB는 여러 도메인을 분기하는 도면 라우팅 overlay이므로 별도 동기화한다.
